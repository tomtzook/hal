...
hal_error_t hal_aio_get(hal_env_t* env, hal_handle_t handle, uint32_t* value) {
    ...
    if (port_node->open_port.type != HAL_TYPE_ANALOG_OUTPUT && port_node->open_port.type != HAL_TYPE_ANALOG_INPUT) {
    ...
    TRACE_INFO("Reading from AIO port %s (handle %u)", port_node->open_port.name, handle);

    status = env->backend.aio_get(env, &port_node->open_port, value);
...
hal_error_t hal_aio_set(hal_env_t* env, hal_handle_t handle, uint32_t value) {
    ...
    if (port_node->open_port.type != HAL_TYPE_ANALOG_OUTPUT) {
        HAL_JUMP_IF_ERROR(HAL_ERROR_OPERATION_NOT_SUPPORTED_FOR_TYPE, end);
    }
    ...
    TRACE_INFO("Writing %d to AIO port %s (handle %u)", value, port_node->open_port.name, handle);

    status = env->backend.aio_set(env, &port_node->open_port, value);
