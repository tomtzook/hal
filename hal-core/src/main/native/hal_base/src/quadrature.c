...
hal_error_t hal_quadrature_get_position(hal_env_t* env, hal_handle_t handle, uint32_t* value) {
    ...
    if (port_node->open_port.type != HAL_TYPE_QUADRATURE) {
        HAL_JUMP_IF_ERROR(HAL_ERROR_OPERATION_NOT_SUPPORTED_FOR_TYPE, end);
    }

    if (env->backend.quadrature_get_pos == NULL) {
        TRACE_ERROR("BACKEND does not support QUADRATURE GETPOS");
        HAL_JUMP_IF_ERROR(HAL_ERROR_UNSUPPORTED_OPERATION, end);
    }

    TRACE_INFO("Reading position from QUADRATURE port %s (handle %u)", port_node->open_port.name, handle);

    status = env->backend.quadrature_get_pos(env, &port_node->open_port, value);
...
hal_error_t hal_quadrature_set_position(hal_env_t* env, hal_handle_t handle, uint32_t value) {
    ...
    if (env->backend.quadrature_set_pos == NULL) {
        TRACE_ERROR("BACKEND does not support QUADRATURE SETPOS");
        HAL_JUMP_IF_ERROR(HAL_ERROR_UNSUPPORTED_OPERATION, end);
    }

    TRACE_INFO("Writing position to QUADRATURE port %s (handle %u)", port_node->open_port.name, handle);

    status = env->backend.quadrature_set_pos(env, &port_node->open_port, value);
...
hal_error_t hal_quadrature_get_period(hal_env_t* env, hal_handle_t handle, uint32_t* value) {
    ...
    if (env->backend.quadrature_get_period == NULL) {
        TRACE_ERROR("BACKEND does not support QUADRATURE GETPERIOD");
        HAL_JUMP_IF_ERROR(HAL_ERROR_UNSUPPORTED_OPERATION, end);
    }

    TRACE_INFO("Reading period from QUADRATURE port %s (handle %u)", port_node->open_port.name, handle);

    status = env->backend.quadrature_get_period(env, &port_node->open_port, value);
